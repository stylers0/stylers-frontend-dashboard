...
export function clipDataToShiftWindow(
  ...
  for (const record of data) {
    ...
    // Entirely before window → drop
    if (recEndMs <= windowStartMs) continue;

    // Entirely inside window → keep as-is
    if (recStartMs >= windowStartMs) {
      result.push(record);
      continue;
    }

    // Straddles the window start → clip
    const clippedDuration = Math.max(0, (recEndMs - windowStartMs) / 1000);
    if (clippedDuration <= 0) continue;

    result.push({
      ...record,
      timestamp: new Date(windowStartMs).toISOString(),
      durationSeconds: Math.round(clippedDuration),
    });
  }
